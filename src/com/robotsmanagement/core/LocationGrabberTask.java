...
public class LocationGrabberTask extends AsyncTask<CustomListItem, Void, Void> {
	...
	protected Void doInBackground(CustomListItem... params) {
		...
		try {
			...
			Log.i(tag,
					String.format(
							"Current location: X: %e, Y: %e, Alfa: %e, P: %e, TimeStamp: %e",
							lok.getX(), lok.getY(), lok.getAngle(), lok.getP(),
							lok.getTimeStamp()));
		} catch (IOException e) {
